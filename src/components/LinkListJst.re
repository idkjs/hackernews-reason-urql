...
let {response} =
    useSubscription(~request=Queries.NEW_VOTES_SUBSCRIPTION.make(), ~handler=Handler(handler));
let {response} =
    useSubscription(~request=Queries.NEW_LINKS_SUBSCRIPTION.make(), ~handler=Handler(handler));
  let nextPage =
    React.useCallback1(
      () =>
        page <= count / 10
